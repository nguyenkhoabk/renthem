...
WPSEO_Local_Admin_Wrappers::textinput( 'show_route_label', __( '"Show route" label', 'yoast-local-seo' ) );
	...
	foreach( $tools_array as $key => $label ) {
		...
		switch( $key ) {
			case 'lseo':
				...
				echo '<p>' . sprintf( __('To receive a %s14 Day FREE Trial%s, please go to %s.', 'yoast-local-seo'), '<strong>', '</strong>', '<a href="http://yoa.st/lseo" target="_blank">www.lseo.com/getstarted</a>' ) . '</p>';
				echo '<iframe name="yoast_lseo" src="http://lseo.com/widget/2/1008/' . esc_attr( $_SERVER['HTTP_HOST'] ) . '"  height="165px" width="100%" scrolling="no" border="0" frameborder="0" allowtransparency="true"></iframe>';
