		...
		function add_mce_popup(){
		    ...
		    <script>
			    function WPSEO_InsertMap() {
			        ...
			        window.send_to_editor("[wpseo_map " + id + " width=\"" + width + "\" height=\"" + height + "\" zoom=\"" + zoom + "\" map_style=\"" + map_style + "\"" + scrollable + show_route + show_state + show_country + show_url + "]");
			    }
		        function WPSEO_InsertAddress() {
		            ...
		            if( location_id == 'all' ) {
		            	...
		            	console.log( jQuery("#wpseo_address_all_locations_orderby") );
		            	console.log( jQuery("#wpseo_address_all_locations_orderby").val() );
		            	orderby = ' orderby=' + jQuery("#wpseo_address_all_locations_orderby").val();
		            	order = ' order=' + jQuery("#wpseo_address_all_locations_order").val();
		            }

		            window.send_to_editor("[" + shortcode_name + " " + id + hide_name + oneline + show_state + show_country + show_phone +show_phone_2 + show_fax + show_email + show_url + show_opening_hours + hide_closed + orderby + order + "]");
		        }
		        ...
		        function WPSEO_InsertStorelocator() {
		            ...
		            window.send_to_editor("[wpseo_storelocator " + show_map + scrollable + show_radius + show_nearest_suggestion + radius + show_filter + "\" map_style=\"" + map_style + "\"" + oneline + show_state + show_country + show_phone +show_phone_2 + show_fax + show_email + show_url + show_opening_hours + hide_closed + "]");
		        }
		        ...
		        <div class="wrap">
		            <div>
		                ...
		                <div style="padding:15px 15px 0 15px;">
		                	...
		                	<br>
		                	<input type="checkbox" id="wpseo_map_scrollable" checked="checked" /> <label for="wpseo_map_scrollable"><?php _e('Allow to scroll and zoom in the map', 'yoast-local-seo'); ?></label><br>
		            ...
		            <div>
		                ...
		                <div style="padding:15px 15px 0 15px;">
		                    <select id="wpseo_address_location_id" onchange="WPSEO_Address_Change_Order( this );">
		                        <option value="">  -- <?php _e('Select a location', 'yoast-local-seo'); ?>  -- </option>
		                        <option value="all"><?php _e('Show all locations', 'yoast-local-seo'); ?></option>
		                        <?php
		                            $locations = get_posts( array(
	                					'post_type'      => 'wpseo_locations',
	                					'posts_per_page' => -1,
		                    ...
		                    <div id="wpseo_address_all_locations_order_wrapper" style="display: none;">
		                    	...
		                    	<select name="wpseo_address_all_locations_orderby" id="wpseo_address_all_locations_orderby">
		                    		...
		                    		<option value="date"><?php _e( 'Chronical', 'yoast-local-seo' ); ?></option>
		                    	</select><br>
		        ...
		        <div class="wrap">
		            <div>
		                ...
		                <div style="padding:15px 15px 0 15px;">
		                	...
		                	<label for="wpseo_sl_scrollable"><input type="checkbox" id="wpseo_sl_scrollable" checked="checked" /> <?php _e('Allow to scroll and zoom in the map', 'yoast-local-seo'); ?></label><br>
		                	...
		                	<br>
		                	<label for="wpseo_sl_radius"><?php printf( __('Search radius (in %s)', 'yoast-local-seo'), $this->options['unit_system'] == 'METRIC' ? 'km' : 'mi' ); ?> <input type="text" id="wpseo_sl_radius" value="10" /></label><br>
		                </div>
