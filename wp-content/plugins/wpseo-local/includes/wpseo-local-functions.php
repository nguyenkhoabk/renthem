...
function wpseo_local_show_address( $atts ) {
	...
	if ( $atts['show_phone'] && ! empty( $business_phone ) ) {
		$output .= '<span class="wpseo-phone">' . __( 'Phone', 'yoast-local-seo' ) . ': <a href="tel:' . $business_phone . '" class="tel" itemprop="telephone">' . $business_phone . '</a></span>' . ( $atts['oneline'] ? ' ' : '<br/>' );
	}

	if ( $atts['show_phone_2'] && ! empty( $business_phone_2nd ) ) {
		$output .= '<span class="wpseo-phone2nd">' . __( 'Secondary phone', 'yoast-local-seo' ) . ': <a href="tel:' . $business_phone_2nd . '" class="tel">' . $business_phone_2nd . '</a></span>' . ( $atts['oneline'] ? ' ' : '<br/>' );
	}

	if ( $atts['show_fax'] && ! empty( $business_fax ) ) {
		$output .= '<span class="wpseo-fax">' . __( 'Fax', 'yoast-local-seo' ) . ': <span class="tel" itemprop="faxNumber">' . $business_fax . '</span></span>' . ( $atts['oneline'] ? ' ' : '<br/>' );
	}

	if ( $atts['show_email'] && ! empty( $business_email ) ) {
		$output .= '<span class="wpseo-email">' . __( 'Email', 'yoast-local-seo' ) . ': <a href="mailto:' . $business_email . '" itemprop="email">' . $business_email . '</a></span>' . ( $atts['oneline'] ? ' ' : '<br/>' );
	}

	if ( $atts['show_url'] ) {
		$output .= '<span class="wpseo-url">' . __( 'URL', 'yoast-local-seo' ) . ': <a href="' . $business_url . '" itemprop="url">' . $business_url . '</a></span>' . ( $atts['oneline'] ? ' ' : '<br/>' );
	}
...
function wpseo_local_get_address_format( $business_zipcode = '', $business_city = '', $business_state = '', $show_state = false, $escape_output = false, $use_tags = true ) {
	...
	if ( in_array( $address_format, array( '', 'address-state-postal', 'address-state-postal-comma', 'address-postal-city-state', 'address-postal', 'address-postal-comma' ) ) ) {
		...
		if ( ! empty( $business_zipcode ) && $address_format != 'address-postal-city-state' ) {
